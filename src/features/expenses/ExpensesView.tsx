...
export const ExpensesView: React.FC = () => {
  ...
  const handleOpenEditTxModal = (tx: Transaction) => {
    setEditingTx(tx);
    setDefaultTxType(tx.transaction_type);
    setIsTxModalOpen(true);
  };
  ...
  const handleQuickAddCategory = async (catData: Partial<ExpenseCategory>) => {
    ...
    return await saveCategory({
      ...
      type: catData.type || defaultTxType,
      color: catData.color || (defaultTxType === 'income' ? '#10B981' : '#F59E0B'),
    });
  };
  ...
  return (
    <div className="space-y-6 pb-12">
      ...
      {/* Modal Add / Edit Transaction (Component form độc lập siêu mượt) */}
