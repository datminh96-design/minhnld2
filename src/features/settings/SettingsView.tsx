...
export const SettingsView: React.FC = () => {
  ...
  const { isSupabaseConfigured, isDemoUser, profile, isAdmin, user, updateProfile, sendVerificationEmail } = useAuth();
  const supabaseStatus = getSupabaseStatus();

  // Recovery email state in settings
  const [recoveryEmailInput, setRecoveryEmailInput] = useState(profile?.recovery_email || '');
  ...
  useEffect(() => {
    if (profile?.recovery_email) {
      setRecoveryEmailInput(profile.recovery_email);
    }
  ...
  const handleRegisterRecoveryEmail = async (e?: React.FormEvent) => {
    ...
    const clean = recoveryEmailInput.trim().toLowerCase();
    ...
    try {
      ...
      await sendVerificationEmail(clean, profile?.full_name || 'Người dùng', code, {
        isRecovery: true,
        accountEmail: user?.email || profile?.email || clean,
      });

      await updateProfile({
        recovery_email: clean,
        recovery_email_verified: false,
      });

      setShowRecoveryVerifyBox(true);
      setRecoveryCountdown(60);
      addToast(`Đã gửi email xác thực tài khoản đến email khôi phục: ${clean}`, 'success');
    } catch (err: any) {
  ...
  const handleVerifyRecoveryOtp = async (e: React.FormEvent) => {
    ...
    setRecoveryOtpInput('');
    addToast(`Xác thực thành công! Email khôi phục ${clean} đã được kích hoạt bảo vệ tài khoản.`, 'success');
  };
...
-- Đã có sẵn trong file supabase/schema.sql của dự án.`;
  ...
  return (
    <div className="space-y-6 pb-16 max-w-4xl">
      ...
      {/* 2.5 Email Khôi Phục & Xác Thực Kích Hoạt Tài Khoản */}
      <div className="p-5 sm:p-6 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-xs space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-slate-100 dark:border-slate-800">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400">
                Email dự phòng nhận thông báo xác thực kích hoạt và khôi phục khi quên mật khẩu
              </p>
            </div>
          </div>
          ...
          <div>
            {profile?.recovery_email && profile?.recovery_email_verified ? (
              ...
              <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-medium bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 border border-slate-200 dark:border-slate-700">
                Chưa thiết lập email khôi phục
              </span>
            )}
          </div>
        </div>
        ...
        <p className="text-xs text-slate-500 dark:text-slate-400 leading-relaxed">
          Hệ thống sẽ gửi <strong>Email xác thực tài khoản kèm mã OTP 6 số</strong> đến email khôi phục này. Khi người dùng bấm kích hoạt hoặc nhập OTP, email khôi phục sẽ chính thức được liên kết bảo vệ tài khoản an toàn.
        </p>
            ...
            <div className="flex flex-col sm:flex-row gap-2.5">
              <div className="relative flex-1">
                ...
                <input
                  ...
                  className="w-full pl-9 pr-3.5 py-2 text-xs rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 text-slate-900 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-emerald-500"
                />
              </div>

              <button
                type="submit"
                disabled={isSendingRecoveryEmail || !recoveryEmailInput}
                className="py-2 px-4 rounded-xl bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 text-white font-semibold text-xs transition-all shadow-md shadow-emerald-500/20 disabled:opacity-50 flex items-center justify-center gap-1.5 cursor-pointer whitespace-nowrap"
              >
        ...
        {showRecoveryVerifyBox && (
          <form onSubmit={handleVerifyRecoveryOtp} className="p-4 rounded-xl bg-emerald-50/70 dark:bg-emerald-950/40 border border-emerald-500/40 space-y-3 animate-in fade-in">
            <div className="flex items-start gap-2.5">
              ...
              <div>
                <h6 className="text-xs font-bold text-emerald-950 dark:text-emerald-100">
                  Nhập mã OTP 6 số từ email khôi phục:
                </h6>
            ...
            <div className="flex items-center justify-between pt-1 text-[11px]">
              <button
                ...
                disabled={recoveryCountdown > 0 || isSendingRecoveryEmail}
